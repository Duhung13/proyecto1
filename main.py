...
def cargar_reactivos_desde_api(reactivos_list):
    ...
    if response.status_code == 200:
        ...
        for data in reactivos_data:
            conversiones = []
            for conv in data.get('conversiones_posibles', []):
                conversiones.append({'unidad': conv['unidad'], 'factor': conv['factor']})

            reactivo = Reactivo(
                id=data['id'],  # Incluir el ID
                nombre=data['nombre'],
...
def cargar_experimentos_de_archivo(recetas_list, archivo='experimentos.json'):
    ...
    try:
        with open(archivo, 'r') as f:
            ...
            for data in experimentos_data:
                receta = buscar_receta_por_id(recetas_list, data['receta_id'])
                experimento = Experimento(
                    id=data['id'],
                    receta_id=receta,  
                    personas_responsables=data['personas_responsables'],
    ...
    if response.status_code == 200:
        ...
        for data in recetas_data:
            receta = Receta(
                ...
                valores_a_medir=data['valores_a_medir']
            )
...
def agregar_reactivo(reactivos_list):
    ...
    reactivo = Reactivo(nombre, descripcion, costo, categoria, inventario, unidad_medida, fecha_caducidad)
    reactivos_list.append(reactivo)
...
def mostrar_recetas(recetas):
    ...
    for receta in recetas:
        ...
        for valor in receta.valores_a_medir:
            ...
            print(f" - Límites: {valor['minimo']} - {valor['maximo']}")
